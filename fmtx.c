...
EXTERN FMTX_API_EXPORT FMTX_API_CALL FMTX_MODE_ENUM fmtxEEPROMSetStartupFlag(uint8_t flag, int force)
{
    ...
    if(force)
        return (FMTX_MODE_ENUM)eeprom_usbAccess(&flag, 1, TX_CONFIG_OFFSET + 2+1, 1);
    else
        return FMTX_MODE_OK;
}
...
{
    return (FMTX_MODE_ENUM)eeprom_usbAccess((uint8_t *) data, size, offset, 1);
}

EXTERN FMTX_API_EXPORT FMTX_API_CALL FMTX_MODE_ENUM fmtxEEPROMReadFwChunk(uint8_t *data, int size, uint16_t offset)
{
    return (FMTX_MODE_ENUM)eeprom_usbAccess(data, size, offset, 0);
}
